$(document).ready(function() {
    $.ajax({
        url:`https://api.apixu.com/v1/forecast.json?key=e51fc4c9af264ce69be71045182010&q='hanoi'&days=7`,
        method: 'GET',
        success: function(data) {
            // var img = 'http' + data.current.condition.icon; 
            $('#day0').html(`${data.forecast.forecastday[0].date}`)
            $('.location').html(`Hanoi`)
            $('#temp0').html(`${data.current.temp_c.toFixed(0)}<sup>o</sup>C`)
            ...
            $('#wind0').html(`<img src="images/icon-wind.png" alt="">${data.forecast.forecastday[0].day.maxwind_kph.toFixed(0)}km/h`)
